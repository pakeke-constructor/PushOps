...
local EMPTY = {
    max = 0xFFFFFFFFFFFF, -- For uninitialized char spawn calls,
...
for _, wType in pairs(Auxiliary) do
    ...
    assert(not (worldTypes[type][tier]), "Overwrite in module worldType. Assert that no worlds overwrite anything else")
